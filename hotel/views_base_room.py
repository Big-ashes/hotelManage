...
def get_list(request):
    ...
    if no:
        #   到数据库去查找数据
        values = Mo.objects.filter(no__contains=no,status=status)[(page-1)*limit:limit*page].values("id",'status',"updatetime","floorno","floorid__id","floorid__name","room",'room_type_id','room_type_id__vip_price','room_type_id__price','room_type_id__typename')
        datas = list(values)
        total =len(datas)
    else:
        #   到数据库去查找数据
        values = Mo.objects.filter(status=status)[(page-1)*limit:limit*page].values("id",'status',"updatetime","floorno","room","floorid__id","floorid__name",'room_type_id','room_type_id__vip_price','room_type_id__price','room_type_id__typename')
        datas = list(values)
        total =len(datas)
    # 构造返回数据
    if total == 0:
...
def add(request):
    ...
    if u:
        ...
        m.floorno = floorno
        m.status = 0
        m.room_type_id_id = room_type_id
